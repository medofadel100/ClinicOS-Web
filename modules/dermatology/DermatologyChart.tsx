...
export default function DermatologyChart({
  patientId, clinicId, locale, initialEntries
}: {
  patientId: string; clinicId: string; locale: string; initialEntries: PinEntry[]
}) {
  const isAr = locale === 'ar'

  // ─── Body Map State ──────────────────────────────────────
  const getPins = (entries: PinEntry[]): Pin[] => {
    const notes = entries.find(n => n.note_type === 'dermatology_map')
  ...
  // ─── Aesthetics State ────────────────────────────────────
  const [activeTab, setActiveTab] = useState<TabId>('body_map')
  const [aestheticsData, setAestheticsData] = useState<DermatologyAestheticsData>({
    laser_sessions: [], injectables: [], skincare: [], treatments: []
  })
  ...
  useEffect(() => {
    if (activeTab !== 'body_map') {
      import('./actions').then(({ getAestheticsData }) => {
        getAestheticsData(clinicId, patientId).then(data => {
          if (data) setAestheticsData(data)
        })
      })
    }
  }, [activeTab, clinicId, patientId])
  ...
  const handleSavePin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedPin) {
      ...
      updatedPins.push({
        ...
        x: newPinCoords.x, y: newPinCoords.y, view: activeView,
        condition: formData.get('condition') as string,
        severity: formData.get('severity') as 'mild' | 'moderate' | 'severe',
        notes: formData.get('notes') as string,
    ...
    try {
      await upsertDermatologyNote(clinicId, locale, patientId, updatedPins)
      setPins(updatedPins)
  ...
  // ─── Aesthetics Save Handler ─────────────────────────────
  const saveAesthetics = async (data: DermatologyAestheticsData) => {
    setLoading(true)
    try {
  ...
  const updateLaser = (laser_sessions: LaserSession[]) => {
    const updated = { ...aestheticsData, laser_sessions }
    setAestheticsData(updated)
    saveAesthetics(updated)
  }
  const updateInjectables = (injectables: InjectableRecord[]) => {
    const updated = { ...aestheticsData, injectables }
    setAestheticsData(updated)
    saveAesthetics(updated)
  }
  const updateSkincare = (skincare: SkincareRecord[]) => {
    const updated = { ...aestheticsData, skincare }
    setAestheticsData(updated)
    saveAesthetics(updated)
  }
  const updateTreatments = (treatments: TreatmentRecord[]) => {
    const updated = { ...aestheticsData, treatments }
    setAestheticsData(updated)
    saveAesthetics(updated)
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
                <MapPin className="w-5 h-5 text-indigo-400" /> {isAr ? 'خريطة الجسم' : 'Body Map'}
              </h3>
              <p className="text-sm text-slate-400">{isAr ? 'اضغط على أي مكان في الجسم لتسجيل آفة أو علامة' : 'Click anywhere on the body to add a lesion marker'}</p>
            </div>
            <div className="flex bg-black/40 p-1 rounded-xl border border-white/10">
              {(['anterior', 'posterior'] as const).map(view => (
                <button key={view} onClick={() => setActiveView(view)}
                  className={`px-6 py-2 rounded-lg text-sm font-medium transition-all ${
                    activeView === view ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/25' : 'text-slate-400 hover:text-white hover:bg-white/5'
                  }`}>
                  {view === 'anterior' ? (isAr ? 'أمامي' : 'Front') : (isAr ? 'خلفي' : 'Back')}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 relative bg-[#0f172a]/50 rounded-2xl border border-white/10 overflow-hidden flex items-center justify-center p-8 min-h-[500px]">
              ...
              <div ref={mapRef} onClick={handleMapClick} className="relative w-full max-w-sm aspect-[3/4] cursor-crosshair">
                <BodyMapSVG />
                {pins.filter(p => p.view === activeView).map(pin => (
                  <button key={pin.id} onClick={(e) => handlePinClick(e, pin)}
                ...
                {newPinCoords && !selectedPin && (
                  <div className="absolute w-5 h-5 -ml-2.5 -mt-2.5 rounded-full bg-white animate-pulse shadow-[0_0_15px_rgba(255,255,255,0.8)] pointer-events-none"
              ...
              <div className="absolute bottom-4 left-4 right-4 flex justify-between text-xs text-slate-500 font-medium uppercase tracking-wider">
                ...
                <span>{pins.filter(p => p.view === activeView).length} {isAr ? 'علامة' : 'Markers'}</span>
              </div>
            </div>

            <div className="bg-[#0f172a]/50 rounded-2xl border border-white/10 p-5 flex flex-col h-[500px]">
              <h4 className="text-sm font-semibold text-slate-200 mb-4 uppercase tracking-wider">{isAr ? 'الآفات المسجلة' : 'Recorded Lesions'}</h4>
              <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                {pins.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 opacity-60">
                    <AlertCircle className="w-10 h-10 mb-2" />
                    <p className="text-sm">{isAr ? 'لا توجد آفات مسجلة' : 'No lesions recorded yet'}</p>
                    <p className="text-xs mt-1">{isAr ? 'اضغط على خريطة الجسم' : 'Click the body map'}</p>
                  </div>
                ) : (
                  pins.map(pin => (
                    <div key={pin.id} onClick={(e) => handlePinClick(e, pin)}
                      ...
                      <div className="text-xs text-slate-400 mb-2 line-clamp-1">{pin.notes || '—'}</div>
      ...
      {/* ─── Laser Tab ─────────────────────────────────────── */}
      {activeTab === 'laser' && (
        <LaserSessions sessions={aestheticsData.laser_sessions} onUpdate={updateLaser} isAr={isAr} loading={loading} />
      )}

      {/* ─── Injectables Tab ───────────────────────────────── */}
      {activeTab === 'injectables' && (
        <Injectables records={aestheticsData.injectables} onUpdate={updateInjectables} isAr={isAr} loading={loading} />
      )}

      {/* ─── Skincare Tab ──────────────────────────────────── */}
      {activeTab === 'skincare' && (
        <SkincareProcedures records={aestheticsData.skincare} onUpdate={updateSkincare} isAr={isAr} loading={loading} />
      )}
      ...
      {/* ─── Pin Dialog ────────────────────────────────────── */}
      <Dialog open={pinDialogOpen} onOpenChange={(open) => { setPinDialogOpen(open); if (!open) { setNewPinCoords(null); setSelectedPin(null) } }}>
        <DialogContent className="bg-[#0a0f1e] border border-white/10 text-white sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{selectedPin ? (isAr ? 'تعديل' : 'Edit') : (isAr ? 'تسجيل جديد' : 'Record New')}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSavePin} className="space-y-4 mt-4">
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">{isAr ? 'الحالة' : 'Condition'}</label>
              <select name="condition" defaultValue={selectedPin?.condition || 'Rash / Erythema'}
                ...
                {['Rash / Erythema', 'Mole / Nevus', 'Eczema / Dermatitis', 'Psoriasis', 'Acne', 'Melanoma / Suspect', 'Burn', 'Skin Tag', 'Other'].map(c =>
                  <option key={c} value={c}>{c}</option>
                )}
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">{isAr ? 'الشدة' : 'Severity'}</label>
              <div className="grid grid-cols-3 gap-2">
                {(['mild', 'moderate', 'severe'] as const).map(sev => (
                  <label key={sev} className={`flex items-center justify-center p-2 rounded-lg border cursor-pointer transition-all text-xs font-medium uppercase tracking-wider ${
                    ...
                    <input type="radio" name="severity" value={sev} defaultChecked={selectedPin?.severity === sev || (!selectedPin && sev === 'mild')} className="hidden" />
                    {sev}
                  </label>
                ))}
              </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">{isAr ? 'ملاحظات' : 'Notes'}</label>
              <textarea name="notes" defaultValue={selectedPin?.notes || ''} rows={3}
                placeholder={isAr ? 'صف الحجم، اللون، الملمس...' : 'Describe size, color, texture...'}
                className="w-full p-3 rounded-lg text-sm bg-black/20 border border-white/10 text-white placeholder:text-slate-600 focus:outline-none focus:border-indigo-500/50 resize-none" />
            </div>
            <div className="flex gap-3 pt-4 mt-6 border-t border-white/10">
              {selectedPin && (
                <button type="button" onClick={() => handleDeletePin(selectedPin.id)} disabled={loading}
