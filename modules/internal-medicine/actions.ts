...
export async function upsertInternalMedicineNote(clinicId: string, locale: string, patientId: string, notes: any[]) {
  ...
  const { data: existingNote } = await supabase
    ...
    .eq('clinic_id', clinicId)
    .eq('note_type', 'internal_medicine_notes')
    .single()
  ...
  if (existingNote) {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        note_type: 'internal_medicine_notes',
        content: { notes }
      })
    if (error) throw error
  }
