...
export async function upsertEndocrinologyNote(clinicId: string, locale: string, patientId: string, readings: any[]) {
  ...
  const { data: existingNote } = await supabase
    ...
    .eq('clinic_id', clinicId)
    .eq('note_type', 'endocrinology_tracker')
    .single()

  if (existingNote) {
    const { error } = await supabase
      .from('patient_clinical_notes')
      .update({ content: { readings }, updated_at: new Date().toISOString() })
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        note_type: 'endocrinology_tracker',
        content: { readings }
      })
    if (error) throw error
  }
