...
export async function upsertPsychiatryNote(clinicId: string, locale: string, patientId: string, records: any[]) {
  ...
  const { data: existingNote } = await supabase
    ...
    .eq('clinic_id', clinicId)
    .eq('note_type', 'psychiatry_tracker')
    .single()

  if (existingNote) {
    const { error } = await supabase
      .from('patient_clinical_notes')
      .update({ content: { records }, updated_at: new Date().toISOString() })
