...
export default function CardiologyChart({
  patientId,
  clinicId,
  locale,
  initialEntries
  ...
  const handleSavePin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const updatedNotes = await upsertCardiologyNote(clinicId, locale, patientId, updatedPins)
  ...
  const HeartMapSVG = () => (
    <svg viewBox="0 0 200 200" className="w-full h-full drop-shadow-xl" preserveAspectRatio="xMidYMid meet">
      {/* Vena Cava */}
      <rect x="60" y="20" width="20" height="40" rx="10" fill="rgba(59,130,246,0.3)" stroke="rgba(59,130,246,0.6)" strokeWidth="2" />
      {/* Aorta */}
      <path d="M100 20 Q130 10 140 40 L140 70 L100 70 Z" fill="rgba(239,68,68,0.3)" stroke="rgba(239,68,68,0.6)" strokeWidth="2" />
      {/* Pulmonary Artery */}
      <path d="M140 40 Q160 50 160 80 L120 80 Z" fill="rgba(59,130,246,0.3)" stroke="rgba(59,130,246,0.6)" strokeWidth="2" />
      
      {/* Right Atrium */}
      <circle cx="70" cy="80" r="25" fill="rgba(59,130,246,0.15)" stroke="rgba(59,130,246,0.4)" strokeWidth="2" />
      {/* Left Atrium */}
      <circle cx="130" cy="80" r="25" fill="rgba(239,68,68,0.15)" stroke="rgba(239,68,68,0.4)" strokeWidth="2" />
      
      {/* Right Ventricle */}
      <ellipse cx="80" cy="130" rx="35" ry="45" fill="rgba(59,130,246,0.1)" stroke="rgba(59,130,246,0.3)" strokeWidth="2" transform="rotate(-15 80 130)" />
      {/* Left Ventricle */}
      <ellipse cx="120" cy="130" rx="40" ry="50" fill="rgba(239,68,68,0.1)" stroke="rgba(239,68,68,0.3)" strokeWidth="2" transform="rotate(15 120 130)" />
      
      {/* Septum Line */}
      <path d="M100 70 L100 170" stroke="rgba(255,255,255,0.2)" strokeWidth="3" strokeDasharray="5,5" />
      
      {/* Coronary Arteries (Abstract lines) */}
      <path d="M120 80 Q130 120 100 160" fill="none" stroke="rgba(239,68,68,0.6)" strokeWidth="2" />
      <path d="M80 80 Q70 120 90 150" fill="none" stroke="rgba(59,130,246,0.6)" strokeWidth="2" />
    </svg>
  )
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
            <span className="text-xl">🫀</span> Cardiology Map
          </h3>
          <p className="text-sm text-slate-400">Click anywhere on the heart diagram to add a murmur, blockage, or anomaly.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 relative bg-[#0f172a]/50 rounded-2xl border border-white/10 overflow-hidden flex items-center justify-center p-8 min-h-[500px]">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex justify-between text-xs text-slate-500 font-medium uppercase tracking-wider">
            ...
            <span>{pins.length} Anomalies Marked</span>
          </div>
        </div>

        <div className="bg-[#0f172a]/50 rounded-2xl border border-white/10 p-5 flex flex-col h-[500px]">
          <h4 className="text-sm font-semibold text-slate-200 mb-4 uppercase tracking-wider">Recorded Anomalies</h4>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
            {pins.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 opacity-60">
                ...
                <p className="text-xs mt-1">Click the heart map to add one.</p>
              </div>
            ) : (
              pins.map(pin => (
                <div 
      ...
      >
        <DialogContent className="bg-[#0a0f1e] border border-white/10 text-white sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{selectedPin ? 'Edit Anomaly Details' : 'Record New Anomaly'}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSavePin} className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <select 
                name="condition"
                defaultValue={selectedPin?.condition || 'Murmur'}
              ...
              >
                <option value="Murmur">Murmur (Systolic/Diastolic)</option>
                <option value="Blockage / Stenosis">Blockage / Stenosis</option>
                <option value="Valve Regurgitation">Valve Regurgitation</option>
                <option value="Ischemia / Infarction">Ischemia / Infarction (Scar)</option>
                <option value="Stent Placement">Stent Placement</option>
                <option value="Arrhythmia Focus">Arrhythmia Focus</option>
            ...
            <div className="space-y-2">
              ...
              <textarea 
                ...
                disabled={!isOwnerOrDoctor}
                placeholder="Describe characteristics, sound, or specific location..."
