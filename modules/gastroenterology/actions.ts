...
export async function upsertGastroNote(clinicId: string, locale: string, patientId: string, content: Record<string, any>) {
  ...
  const { data: existing } = await supabase
    ...
    .eq('clinic_id', clinicId)
    .eq('note_type', 'gastroenterology_notes')
  ...
  if (existing) {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        note_type: 'gastroenterology_notes',
        content
      })
    if (error) throw error
  }
...
export async function deleteGastroNote(clinicId: string, noteId: string) {
