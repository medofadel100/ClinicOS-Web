...
import { upsertGastroNote, deleteGastroNote } from './actions'
...
export default function GIExam({ clinicId, locale, patientId, initialEntries }: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await upsertGastroNote(clinicId, locale, patientId, content)
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id)
    try {
      await deleteGastroNote(clinicId, id)
    ...
    <div className="space-y-6">
      <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05]">
        <h3 className="text-sm font-semibold text-slate-300 mb-4 flex items-center gap-2">
          ...
          {isAr ? 'إضافة كشف الجهاز الهضمي' : 'Add New GI Exam Record'}
        </h3>
