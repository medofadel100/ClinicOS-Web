...
export async function upsertPhysioNote(clinicId: string, locale: string, patientId: string, content: Record<string, any>) {
  ...
  const { data: existing } = await supabase
    ...
    .eq('clinic_id', clinicId)
    .eq('note_type', 'physical_therapy_notes')
  ...
  if (existing) {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        note_type: 'physical_therapy_notes',
        content
      })
    if (error) throw error
  }
...
export async function deletePhysioNote(clinicId: string, noteId: string) {
