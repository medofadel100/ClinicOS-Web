...
interface DermatologyModuleProps {
  ...
  clinicId: string;
  clinicalNotes: any[];
  onUpdate: (type: string, content: any) => Promise<void>;
  isOwnerOrDoctor: boolean;
}
...
export default function DermatologyModule({
  ...
  // Sync with props
  useEffect(() => {
    const latest = clinicalNotes.find(n => n.note_type === 'dermatology_map')
    if (latest) {
      setPins(latest.content?.pins || [])
    }
  }, [clinicalNotes])
  ...
  const handleMapClick = (e: React.MouseEvent) => {
    if (!isOwnerOrDoctor || selectedPin) return; // Don't add pin if viewing a pin
  ...
  const handleSavePin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onUpdate('dermatology_map', { pins: updatedPins })
      setPins(updatedPins)
  ...
  const getSeverityColor = (sev: string) => {
    if (sev === 'severe') return 'bg-red-500 shadow-red-500/50'
    if (sev === 'moderate') return 'bg-amber-500 shadow-amber-500/50'
    return 'bg-blue-500 shadow-blue-500/50'
  }

  // Abstract Body SVG Component
  const BodyMapSVG = () => (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 relative bg-[#0f172a]/50 rounded-2xl border border-white/10 overflow-hidden flex items-center justify-center p-8 min-h-[500px]">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex justify-between text-xs text-slate-500 font-medium">
            <span>{activeView.toUpperCase()} VIEW</span>
            ...
            {pins.length === 0 ? (
              ...
              pins.map(pin => (
                ...
                >
                  ...
                  <div className="text-xs text-slate-400 mb-1 line-clamp-1">{pin.notes || 'No notes provided'}</div>
                  <div className="text-[10px] text-slate-500 font-mono flex justify-between">
                    <span>{pin.view.toUpperCase()}</span>
        ...
        <DialogContent className="bg-[#0a0f1e] border-white/10 text-white sm:max-w-[425px]">
          ...
          <form onSubmit={handleSavePin} className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <select 
                name="condition"
                defaultValue={selectedPin?.condition || 'Rash'}
              ...
              >
                <option value="Rash">Rash / Erythema</option>
                <option value="Mole">Mole / Nevus</option>
                <option value="Eczema">Eczema / Dermatitis</option>
                <option value="Psoriasis">Psoriasis</option>
                <option value="Acne">Acne</option>
                <option value="Melanoma">Melanoma / Suspect</option>
