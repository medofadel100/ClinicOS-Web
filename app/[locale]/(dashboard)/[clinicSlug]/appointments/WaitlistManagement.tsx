  ...
  return (
    <PremiumCard>
      <div className="flex flex-row items-center justify-between mb-5 pb-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div>
          <h2 className="text-base font-semibold text-slate-200">{isAr ? 'قائمة الانتظار' : 'Waitlist'}</h2>
          <p className="text-sm text-slate-500 mt-0.5">{isAr ? 'إدارة المرضى بانتظار الإلغاءات.' : 'Manage patients waiting for cancellations.'}</p>
        </div>
        <AddWaitlistDialog 
          clinicId={clinicId} 
          locale={locale} 
      ...
      <div>
        {waitlist.length === 0 ? (
          <p className="text-sm text-slate-500 mt-4 text-center">{isAr ? 'قائمة الانتظار فارغة.' : 'The waitlist is empty.'}</p>
        ) : (
              ...
              >
                <div className="font-semibold flex justify-between">
                  ...
                  <span className={`capitalize text-xs px-2 py-1 rounded-full ${entry.status === 'notified' ? 'bg-teal-500/20' : 'bg-slate-500/20'}`}>
                    {entry.status === 'notified' ? (isAr ? 'تم الإشعار' : 'Notified') : entry.status}
                  </span>
                </div>
                <div className="text-slate-400 mt-2 text-sm">
                  {isAr ? 'النطاق' : 'Target'}: {entry.desired_from} to {entry.desired_to}
                </div>
                {entry.clinic_staff_memberships && (
                  <div className="text-xs text-slate-500 mt-1">
                    {isAr ? 'الطبيب' : 'Doctor'}: {entry.clinic_staff_memberships.staff_members?.full_name}
                  </div>
                )}
                {entry.status === 'notified' && (
                  <div className="text-xs text-teal-400 mt-2">
                    {isAr ? 'ربما فُتح موعد يطابق هذا الطلب!' : 'A slot may have opened up matching this request!'}
                  </div>
                )}
              </div>
            ))}
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-[#0a0f1e] border-white/10 text-white">
        <DialogHeader>
          <DialogTitle>{isAr ? 'إضافة لقائمة الانتظار' : 'Add to Waitlist'}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          <div className="space-y-2">
            <label className="text-xs font-semibold uppercase tracking-wider text-slate-400" htmlFor="patient_id">{isAr ? 'المريض' : 'Patient'}</label>
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold uppercase tracking-wider text-slate-400" htmlFor="membership_id">{isAr ? 'الطبيب المفضل (اختياري)' : 'Preferred Doctor (Optional)'}</label>
            <select id="membership_id" name="membership_id" className="w-full h-10 px-3 rounded-lg text-sm bg-black/20 border border-white/10 text-white focus:outline-none focus:border-teal-500/50 focus:ring-1 focus:ring-teal-500/50 transition-all">
              <option value="">{isAr ? 'أي طبيب' : 'Any Doctor'}</option>
              {doctors.map((d) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-400" htmlFor="desired_to">{isAr ? 'إلى تاريخ' : 'To Date'}</label>
          ...
          <div className="flex justify-end pt-4 border-t border-white/10">
            ...
            >
              {loading ? (isAr ? 'جاري الإضافة...' : 'Adding...') : (isAr ? 'إضافة لقائمة الانتظار' : 'Add to Waitlist')}
            </button>
          </div>
        </form>
      </DialogContent>
