  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogTrigger asChild>
        <Button variant="outline" size="sm">{isAr ? 'إعادة جدولة' : 'Reschedule'}</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="date">{isAr ? 'التاريخ الجديد' : 'New Date'}</Label>
              <Input id="date" name="date" type="date" defaultValue={initialDate} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="time">{isAr ? 'الوقت الجديد' : 'New Time'}</Label>
              <Input id="time" name="time" type="time" defaultValue={initialTime} required />
            </div>
          </div>

          <Button type="submit" className="w-full" disabled={loading}>
            {loading ? (isAr ? 'جاري إعادة الجدولة...' : 'Rescheduling...') : (isAr ? 'تأكيد' : 'Confirm')}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
