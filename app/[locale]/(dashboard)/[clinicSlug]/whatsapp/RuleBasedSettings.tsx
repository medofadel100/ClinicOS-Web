    ...
    <div className="mt-6">
      <PremiumCard>
        <div className="mb-5 pb-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-base font-semibold text-slate-200">{isAr ? 'إعدادات القواعد' : 'Rule-Based Settings'}</h2>
          <p className="text-sm text-slate-500 mt-0.5">{isAr ? 'إعداد الردود التلقائية للاستفسارات الشائعة.' : 'Configure automated responses for common queries.'}</p>
        </div>
        <div className="space-y-6">
          <div className="space-y-4">
            {options.length === 0 ? (
              <p className="text-sm text-slate-500">{isAr ? 'سيتم إنشاء الخيارات الافتراضية تلقائياً عند أول رسالة من مريض للبوت.' : 'Default options will be generated automatically when the first patient messages the bot.'}</p>
            ) : (
              options.map(opt => (
          ...
          <div className="pt-6 mt-6" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold uppercase tracking-wider text-slate-500">{isAr ? 'الاسم بالعربية' : 'Label (Arabic)'}</label>
                  <input
                    value={newAr}
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-500">{isAr ? 'الرسالة' : 'Response Message'}</label>
                <input
                  value={newResponse}
