      ...
      <PremiumCard>
        <div className="flex items-center justify-between mb-6 pb-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">{isAr ? 'ربط حساب واتساب عبر رمز QR.' : 'Connect your WhatsApp account via QR code.'}</p>
          </div>
          {isConnected ? (
            <span
            ...
            >
              <Wifi className="w-3 h-3" /> {isAr ? 'متصل' : 'Connected'}
            </span>
          ) : (
            <span
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold"
            ...
            >
              <WifiOff className="w-3 h-3" /> {isAr ? 'غير متصل' : 'Disconnected'}
            </span>
          )}
        </div>
        ...
        {isConnected ? (
          <div className="flex items-center justify-between p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div>
              <p className="font-medium text-slate-200">{isAr ? 'الاتصال' : 'Active Session'}</p>
              <p className="text-sm text-slate-400">
                {isAr ? 'الرقم المتصل: ' : 'Connected Number: '}{phone || 'Unknown'}
              </p>
            </div>
            <button
          ...
          <div className="flex flex-col items-center justify-center p-6 rounded-xl space-y-4 text-center" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <h3 className="font-semibold text-lg text-slate-200">{isAr ? 'امسح رمز QR هذا بواتساب' : 'Scan QR Code'}</h3>
            ...
            <div className="flex items-center gap-2 text-sm text-emerald-400">
              ...
              {isAr ? 'بانتظار المسح...' : 'Waiting for scan...'}
            </div>
            <button
          ...
          <div className="flex flex-col items-center justify-center p-8 border border-dashed rounded-xl text-center" style={{ borderColor: 'rgba(255,255,255,0.15)' }}>
            ...
            <h3 className="font-medium text-slate-200 mb-1">{isAr ? 'لا يوجد اتصال' : 'No Active Connection'}</h3>
            <p className="text-sm text-slate-400 mb-6 max-w-md">
              {isAr ? 'ربط حساب واتساب الخاص بعيادتك لبدء إرسال التذكيرات التلقائية والرد على الاستفسارات وتمكين المرضى من حجز مواعيد مباشرة عبر المحادثة.' : 'Connect your WhatsApp Business number to start sending automated reminders, responding to inquiries, and letting patients book appointments directly via chat.'}
            </p>
            <button
