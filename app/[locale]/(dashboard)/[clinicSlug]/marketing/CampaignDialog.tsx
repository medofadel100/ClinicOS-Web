...
export default function CampaignDialog({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : (isAr ? 'فشل في حفظ الحملة' : 'Failed to save campaign'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogTrigger asChild>
        <Button variant={isEditing ? 'outline' : 'default'} size={isEditing ? 'sm' : 'default'}>
          {isEditing ? (isAr ? 'تعديل' : 'Edit') : (isAr ? 'حملة جديدة' : 'New Campaign')}
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{isEditing ? (isAr ? 'تعديل الحملة' : 'Edit Campaign') : (isAr ? 'إنشاء حملة' : 'Create Campaign')}</DialogTitle>
          <DialogDescription>
            {isAr ? 'إعداد تفاصيل الحملة التسويقية لتتبع العائد.' : 'Configure marketing campaign details to track ROI.'}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 pt-4">
          <div className="space-y-2">
            <Label htmlFor="name">{isAr ? 'اسم الحملة' : 'Campaign Name'}</Label>
            <Input id="name" name="name" required defaultValue={campaign?.name} placeholder="e.g. Summer Special 2026" />
          </div>

          <div className="space-y-2">
            <Label htmlFor="platform">{isAr ? 'المنصة' : 'Platform'}</Label>
            <Select name="platform" defaultValue={campaign?.platform || 'Facebook'}>
              ...
              <SelectContent>
                ...
                <SelectItem value="Offline">{isAr ? 'غير إلكتروني / مطبوع' : 'Offline / Print'}</SelectItem>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="end_date">{isAr ? 'تاريخ الانتهاء' : 'End Date'}</Label>
              <Input id="end_date" name="end_date" type="date" defaultValue={campaign?.end_date || ''} />
            </div>
          </div>

          <div className="space-y-2">
            <Label htmlFor="budget_egp">{isAr ? 'الميزانية (ج.م)' : 'Budget (EGP)'}</Label>
          ...
          <div className="flex items-center space-x-2 pt-2">
            ...
            <Label htmlFor="is_active">{isAr ? 'الحملة نشطة' : 'Campaign is Active'}</Label>
          </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading}>
              {loading ? (isAr ? 'جاري الحفظ...' : 'Saving...') : (isAr ? 'حفظ الحملة' : 'Save Campaign')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
