  ...
  if (state.step === 'menu') {
    ...
    switch (option.response_type) {
      ...
      case 'action_book':
        ...
        // Fetch services
        const { data: services } = await supabase.from('clinic_services').select('*').eq('clinic_id', clinicId)
        ...
        services.forEach((s, idx) => {
          svcText += `${idx + 1}. ${(s as { name?: string }).name}\n`
        })
      ...
      case 'action_cancel':
        ...
        const { data: apps } = await supabase
          .from('appointments')
          .select('id, scheduled_at, clinic_services(name)')
          .eq('clinic_id', clinicId)
        ...
        apps.forEach((a, idx) => {
          cancelText += `${idx + 1}. ${(a.clinic_services as { name?: string } | null)?.name} at ${new Date(a.scheduled_at).toLocaleString()}\n`
        })
    ...
    const { data: services } = await supabase.from('clinic_services').select('id').eq('clinic_id', clinicId)
